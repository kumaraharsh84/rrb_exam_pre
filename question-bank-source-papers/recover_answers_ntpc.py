...
PDF_DIR = BASE / "ntpc"
SOURCE_DIR = BASE / "quality-gated-json" / "ntpc"
OUT_DIR = BASE / "answer-recovered-json" / "ntpc"
WORK_ROOT = BASE / "answer-recovery-tests" / "ntpc"
SUMMARY_PATH = OUT_DIR / "ntpc-answer-recovery-summary.json"
...
def colored_option_rows(image_path: Path, page: int) -> list[dict]:
    ...
    crop_x2 = min(width, 900)
    cropped = image.crop((80, 0, crop_x2, height))
    data = cropped.getdata()
    cropped_width = crop_x2 - 80
