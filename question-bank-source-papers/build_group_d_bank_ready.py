...
SOURCE_DIR = BASE / "answer-recovered-json" / "group-d"
OUT_DIR = BASE / "bank-ready-json" / "group-d"
COMBINED_OUT = OUT_DIR / "group-d-2025-bank-ready.json"
SUMMARY_OUT = OUT_DIR / "group-d-2025-bank-ready-summary.json"
...
def make_bank_id(question: dict) -> str:
    ...
    paper = paper.replace("rrb-group-d-cbt-1-paper-", "")
    paper = re.sub(r"[^a-z0-9]+", "_", paper).strip("_")
    return f"rrb_group_d_{paper}_q{question['question_number']:03d}"
...
def main() -> None:
    ...
    payload = {
        "exam": "RRB Group D",
        "language": "English",
        "source_scope": "2025 English Group D papers",
    ...
    for subject in sorted(subject_counts):
        ...
        (OUT_DIR / f"group-d-2025-{subject_slug}.bank-ready.json").write_text(
    ...
    summary = {
        ...
        "output_files": [
            COMBINED_OUT.name,
            *[
                f"group-d-2025-{re.sub(r'[^a-z0-9]+', '-', subject.lower()).strip('-')}.bank-ready.json"
                for subject in sorted(subject_counts)
            ],
            SUMMARY_OUT.name,
        ],
