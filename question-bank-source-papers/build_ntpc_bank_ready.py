...
SOURCE_DIR = BASE / "answer-recovered-json" / "ntpc"
OUT_DIR = BASE / "bank-ready-json" / "ntpc"
COMBINED_OUT = OUT_DIR / "ntpc-2024-bank-ready.json"
SUMMARY_OUT = OUT_DIR / "ntpc-2024-bank-ready-summary.json"
...
def make_bank_id(question: dict) -> str:
    ...
    paper = paper.replace("rrb-ntpc-cbt-1-paper-", "")
    paper = re.sub(r"[^a-z0-9]+", "_", paper).strip("_")
    return f"rrb_ntpc_{paper}_q{question['question_number']:03d}"
...
def main() -> None:
    ...
    payload = {
        "exam": "RRB NTPC",
        "language": "English",
        "source_scope": "2024 English NTPC papers",
    ...
    for subject in sorted(subject_counts):
        ...
        (OUT_DIR / f"ntpc-2024-{subject_slug}.bank-ready.json").write_text(
    ...
    summary = {
        ...
        "output_files": [
            COMBINED_OUT.name,
            *[
                f"ntpc-2024-{re.sub(r'[^a-z0-9]+', '-', subject.lower()).strip('-')}.bank-ready.json"
                for subject in sorted(subject_counts)
            ],
            SUMMARY_OUT.name,
        ],
