...
SOURCE_FILE = BASE / "duplicate-audit-json" / "group-d" / "group-d-2025-deduped-preview.json"
FRONTEND_BANK = PROJECT_ROOT / "frontend" / "data" / "question-bank.json"
OUT_DIR = BASE / "app-merge-json" / "group-d"
APP_MERGE_OUT = OUT_DIR / "group-d-2025-app-merge.json"
MERGED_PREVIEW_OUT = OUT_DIR / "question-bank-with-group-d-2025-preview.json"
SUMMARY_OUT = OUT_DIR / "group-d-2025-app-merge-summary.json"
...
def main() -> None:
    ...
    existing_bank = []
    if FRONTEND_BANK.exists():
        existing_bank = json.loads(FRONTEND_BANK.read_text(encoding="utf-8-sig"))
    ...
    app_questions.sort(key=lambda item: (item["year"] or 0, item["shift"] or "", item["subject"], item["topic"] or "", item["id"]))
