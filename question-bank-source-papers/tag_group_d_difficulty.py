...
SOURCE_DIR = BASE / "topic-tagged-json" / "group-d"
OUT_DIR = BASE / "difficulty-tagged-json" / "group-d"
SOURCE_FILE = SOURCE_DIR / "group-d-2025-topic-tagged.json"
COMBINED_OUT = OUT_DIR / "group-d-2025-difficulty-tagged.json"
SUMMARY_OUT = OUT_DIR / "group-d-2025-difficulty-tagging-summary.json"
...
def main() -> None:
    ...
    for subject, questions in sorted(by_subject.items()):
        out_file = OUT_DIR / f"group-d-2025-{subject_slug(subject)}.difficulty-tagged.json"
        subject_payload = {
            "exam": output["exam"],
