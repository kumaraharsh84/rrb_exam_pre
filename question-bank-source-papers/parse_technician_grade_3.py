...
TXT_DIR = BASE / "extracted-text" / "technician-grade-3"
OUT_DIR = BASE / "parsed-json" / "technician-grade-3"
SUMMARY_PATH = OUT_DIR / "technician-grade-3-parse-summary.json"
PDFTOTEXT = Path(
...
def parse_text(txt_path: Path) -> dict:
    ...
    data = {
        ...
        "notes": [
            ...
            "Technician Grade 3 options use A-D labels.",
...
def main() -> None:
    ...
    for pdf_path in sorted(PDF_DIR.glob("*.pdf")):
        if "english" not in pdf_path.name.lower():
            continue
